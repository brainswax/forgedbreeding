...
- **Rump Angle 36** — liked flatter angle; low-heritability enough that loss is hard to recover. Explicit protect against further flattening (higher RA than 36).
...
7. General Appearance `G` — reflects the softer structural letters (Rump `A`, Feet `A`, Back `G`).
...
4. Rump Width — dual purpose (trait itself + angle buffer for a doe whose liked flatter angle you want to keep).
