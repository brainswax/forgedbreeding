# Individual Report: Cade's Lil Farm SW Snickers (PD2380420)

**Short name:** SW Snickers

**Age / Lactation at appraisal:** ~2y 4m / Lactation 1 (appraisal 26-Jul-2026)  
**Final Score:** `GAEV 84`  
**Official LA stature score:** 22  
**Favorite animal:** Yes — protect liked Rump Angle and exceptional body depth.

## Summary
Snickers sits at `GAEV 84` with a distinctive package: extraordinary Body Depth (47) and Body Capacity `E`, plus a liked steeper Rump Angle (36) that must be protected. Final Score is limited by Dairy Strength `A` (Dairyness only 23), narrow Rump Width (23), soft Medial Suspensory Ligament (18) and Teat Placement (20), and soft rear-leg set (Rear Legs Side View 21). Breeding should raise dairyness, MSL, and teats while **preserving Rump Angle and depth** — do not casually trade those for score.

## Strengths
- **Body Depth 47 / Body Capacity `E` / Strength 36** — standout capacity; lock this in.
- **Rump Angle 36** — liked; low-heritability enough that loss is hard to recover. Explicit protect.
- **Fore Udder Attachment 39** — excellent; protect.
- Mammary category still `V` despite weak MSL — some mammary topography is working (Rear Udder Height 31, Arch 32, Udder Depth 33).
- Head `V`, Legs Front `V`.

## Weaknesses / Areas for Improvement
1. **Dairyness 23 / Dairy Strength `A`** — largest category drag unique to Snickers; high-ROI and high-heritability enough to prioritize.
2. **Medial Suspensory Ligament 18** — herd-pattern longevity fault.
3. **Teat Placement 20 / Teat Diameter 21** — soft teats; functional + score priority.
4. **Rump Width 23** — narrow; also weakens her ability to buffer angle changes in offspring.
5. **Rear Legs Side View 21** — soft set (low heritability); Feet `A`, Rump category `A`.
6. **Stature 22** — small; avoid partners that pull stature further down without a clear package reason.
7. General Appearance `G` — reflects the flatter structural letters (Rump `A`, Feet `A`, Back `G`).

## Highest-ROI Improvement Targets
1. Dairyness (unique to her among the does).
2. Medial Suspensory Ligament.
3. Teat Placement / Teat Diameter.
4. Rump Width — dual purpose (trait itself + angle buffer for a doe whose angle you want to keep).
5. Hold Body Depth and Rump Angle constant as non-negotiable while pursuing the above.

## Lower-Priority or Harder Traits
- Rear Legs Side View / feet — important long-term but slow to move; don’t sacrifice dairyness/MSL gains solely for this.
- Stature — correct only if a mating would push the line clearly outside functional range.
- Rear Udder Arch — intermediate; protect from erosion rather than chase aggressively.

## Breeding Notes
- **Preferred buck type:** High dairyness + better MSL/teats + wider rump, **without flattening Rump Angle**. **Jane’s Finale** (estimated) is a natural dairyness/width candidate for her — still only middling on MSL/teats, so not a complete mammary solution. **Michael Darling** offers balance and strength/capacity reinforcement; verify angle/width package before use. **Smithy** adds stature/strength but is narrow (Rump Width 24) and only moderate dairyness — weaker fit for her two biggest gaps (dairyness + width).
- **Key offspring risks:** Losing the liked Rump Angle (36); losing depth; stacking narrow rumps; fixing soft MSL + teats.
- **Mitigations:** Any flatter-angled buck must bring clear Rump Width; never stack soft dairyness onto her 23; prefer partners that improve MSL/teats if available.
- Favorite: every recommendation should state expected Final Score benefit **and** whether Rump Angle / depth are protected.
- First lactation — mammary may still mature; dairyness and width gaps are already large enough to act on.
